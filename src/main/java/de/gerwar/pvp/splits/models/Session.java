package de.gerwar.pvp.splits.models;
 ...
 * - SplitService owns a Map<String, Session> of all sessions and switches the current active
 * child by storing its id. SplitService also forks new children on roster changes and
 * appends kills to the active child.
 * <p>
 * What each Session holds
 * - id: unique identifier (UUID string) used as the key in SplitService's map and persisted.
 ...
 * SplitService ends the current child and creates a new child with a copied roster and the
 ...
 * - This model is Gson-serializable; SplitService persists an array of Session objects in the
 * SplitsConfigKeys as JSON. Instant fields are serialized via a custom adapter.
 */
@Getter
